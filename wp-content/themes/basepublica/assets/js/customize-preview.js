/* global basepublicaBgColors, basepublicaPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( basepublicaBgColors ).forEach( function( context ) {
					basepublicaGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function basepublicaGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'basepublica-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#basepublica-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( basepublicaPreviewEls[ context ] ) {
				_.each( basepublicaPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		basepublicaGenerateColorA11yPreviewStyles( 'content' );
		basepublicaGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
