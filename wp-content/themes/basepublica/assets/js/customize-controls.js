/* global basepublicaBgColors, basepublicaColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( basepublicaBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( basepublicaBgColors[ context ].color ) {
						backgroundColorValue = basepublicaBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( basepublicaBgColors[ context ].setting ).get();
					}
					basepublicaSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( basepublicaBgColors ).forEach( function( context ) {
			wp.customize( basepublicaBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					basepublicaSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		basepublicaSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				basepublicaSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function basepublicaSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = basepublicaColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Base Publica 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function basepublicaSetRetineLogoVisibility( visible ) {
